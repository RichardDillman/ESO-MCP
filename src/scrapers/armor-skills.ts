...
export class ArmorSkillsScraper extends BaseScraper {
  private readonly DATA_DIR = path.join(process.cwd(), 'data', 'scraped', 'skills');

  // Armor skill line configurations
  private readonly ARMOR_SKILLS = [
    { name: 'Light Armor', page: 'Light_Armor', category: 'armor' },
    { name: 'Medium Armor', page: 'Medium_Armor', category: 'armor' },
    { name: 'Heavy Armor', page: 'Heavy_Armor', category: 'armor' },
  ];

  async scrape(): Promise<void> {
    logger.info('Starting armor skill lines scrape...');

    for (const skillLineConfig of this.ARMOR_SKILLS) {
      try {
        await this.scrapeArmorSkillLine(skillLineConfig);
      } catch (error) {
        logger.error(`Failed to scrape armor skill line ${skillLineConfig.name}:`, error);
      }
    }

    logger.info('All armor skill lines scraped successfully!');
  }

  private async scrapeArmorSkillLine(skillLineConfig: { name: string; page: string; category: string }): Promise<void> {
    logger.info(`Scraping armor skill line: ${skillLineConfig.name}`);
    ...
    const skillLineData: ArmorSkillLineData = {
      id: skillLineId,
  ...
  private parseSkillsFromSkillLinePage($: cheerio.CheerioAPI, skillLineName: string): any[] {
    ...
    $('table.wikitable').each((_, table) => {
      ...
      $table.find('tr').slice(1).each((_, row) => {
        ...
        // Determine skill type (armor skills are typically all passive)
        let skillType = 'passive';
        const rowClass = $row.attr('class') || '';
        const descLower = description.toLowerCase();

        if (descLower.includes('active') || rowClass.includes('active')) {
          skillType = 'active';
